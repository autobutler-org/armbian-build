...
function extension_prepare_config__appliance_image_defaults() {
	...
	: "${APPLIANCE_IMAGE_SYSTEMD_CAPABILITIES:=}"
...
function pre_customize_image__appliance_image_copy_payload() {
	...
	if [[ -n "${APPLIANCE_IMAGE_SYSTEMD_CAPABILITIES}" ]]; then
		echo "AmbientCapabilities=${APPLIANCE_IMAGE_SYSTEMD_CAPABILITIES}" >> "${systemd_unit_path}"
		echo "CapabilityBoundingSet=${APPLIANCE_IMAGE_SYSTEMD_CAPABILITIES}" >> "${systemd_unit_path}"
	fi
...
function post_customize_image__appliance_image_provision() {
	...
	if [[ "${APPLIANCE_IMAGE_ENABLE_UFW}" == "yes" ]]; then
		if chroot_sdcard "ufw --force reset"; then
			...
			display_alert "Extension: ${EXTENSION}" "ufw setup failed in chroot; continuing without firewall configuration" "warn"
		fi
	fi
